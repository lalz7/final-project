...
# --- 1. SETUP LIBRARY ---
try:
    # Load dependency dengan RTLD_GLOBAL (Wajib di Linux)
...
# Struktur Alarmer (Untuk ambil IP Pengirim)
class NET_DVR_ALARMER(Structure):
    _fields_ = [
        ...
        ("sDeviceIP", c_char * 128), # <--- IP ADA DI SINI
...
# --- 3. CLASS DRIVER UTAMA (CTYPES VERSION) ---

class SDKDriver:
    def __init__(self):
        self.sessions = {} # { "IP": { "user_id": int, "alarm_handle": int } }
    ...
    def _internal_callback(self, lCommand, pAlarmer, pAlarmInfo, dwBufLen, pUser):
        ...
        if lCommand == 0x5002:
            try:
                ...
                # --- BEDAH MEMORI (MANUAL OFFSET - HASIL RISET KITA) ---
                ...
                # D. Masker (Offset 532)
                # Byte: 0=No, 1=Yes
                mask_byte = buffer[532]
                if mask_byte == 1: mask_str = "Yes"
                elif mask_byte == 0: mask_str = "No"
                else: mask_str = "Unknown"

                # E. Gambar (Cari Header JPEG FF D8)
                picture_path = None
                start_img = buffer.find(b'\xff\xd8', 600) # Cari mulai dari offset 600 biar aman
                if start_img != -1:
                ...
                print(f"⚡ [REAL-TIME] ID:{auth_id} | 🌡️ {temperature}°C | 😷 {mask_str}")

                # [FIX ERROR VARIABLE] Bungkus dalam dictionary 'final_data'
                final_data = {
                    "device": device_ip,
                    "authId": auth_id,
                    "date": date_str,
                    "picture": picture_path,
                    "temperature": temperature, # Data Suhu
                    "mask": mask_str,           # Data Masker
                    "source": "REALTIME"
                }
    ...
    # --- FITUR CRUD USER DIMATIKAN ---
    
    def set_user_info(self, ip, emp_no, name, start_dt=None, end_dt=None):
        return False

    def set_user_face(self, ip, emp_no, image_data):
        return False

    def del_user(self, ip, emp_no):
        return False
